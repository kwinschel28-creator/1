...
### Promotion-slot ranking: proposed 2026-08-28, NOT adopted

**The problem.** The one-verdict-per-run promotion cap means exactly one name per run gets the guidance check, the gate that has done all the killing (FISV, PODD, TSCO, CSGP, INTU, AMRZ, ONON). Nothing in this file ever specified **how the name filling that slot is chosen**, so in practice it was ranked by dislocation currentness, distance above the 12-month low. That is an unstated parameter, the same defect class as the 8/18 unstated insider-veto window and the 8/26 unreported empty set. Third application of the same lesson.

**Why it is the wrong ranking.** The 2026-08-10 finding says raw drawdown selects for companies that cut guidance in the last month. Ranking the promotion slot by depth and currentness of drawdown therefore hands the single available guidance check, each run, to the name most likely to fail it. ONON and TJX on 8/28 and AMRZ on 8/27 were all selected this way and all died.

**Evidence it cost a real name.** Of the 38 names clearing gates 1 to 6 and 8 at the 2026-08-27 closes, **21 beat on EPS and fell on the print**, so the results-up/price-down half of the PASS condition is common rather than scarce. SEZL cleared all ten gate conditions on the **2026-08-17 adoption list**, was named there as one of the eight that had not had a guidance check, and sat eleven days while the slot went elsewhere. When the check finally ran on 2026-08-28 it returned a PASS: guidance raised to the high end of 35% revenue growth with adjusted net income raised to $185M, on a 13% EPS beat, against a 33.9% single-session decline.

**The proposed rule.** Rank the promotion slot by the PASS condition itself: the size of the EPS beat paired with the size of the price fall on the print. Gate 8 reverts to the display column it was always specified to be and stops driving the queue. **No threshold moves and no gate widens**; the near-miss rule, the insider veto, the WING guard and the guidance kill are all untouched.

**Status: proposed to Kevin 2026-08-28, awaiting his decision.** It changes how the screen ranks its own work queue, which is his to authorise, and it was proposed the same morning he pushed back on idle cash, which is logged here per the goalpost-movement practice rather than glossed. The honest counter to the proposal: ranking by beat-size-plus-fall will systematically surface names that fell on **deceleration** rather than on sentiment, which is the SEZL bear case exactly, so it may trade one failure mode for another. That is a question for the monthly review with a real cohort, not a settled point.

### Gate 7 buy leg does not exclude 10b5-1, and it changed a live read (found 2026-08-31)

**The defect.** Gate 7 is specified as "discretionary selling exceeds buying by more than 10%, AND net selling exceeds $250k." The sell leg names itself discretionary. **The buy leg says only "buying"**, and the implementation inherited the I1/I2 workflow filter `tx_code='P' AND NOT is_auto_plan`, which excludes employer-run ESPP/DRIP programmes but **does NOT exclude `is_10b5_1` pre-scheduled purchases**. The INS path has always required `is_10b5_1=false` on its buy side. The two paths therefore disagree about what a purchase is, and nothing in this file ever said so.

**How it surfaced.** MWA was promoted to the verdict slot on 2026-08-31 and its gate 7 was reported as "$59,984 bought against $75,000 sold, saved by the $250k materiality floor." The deep check the same day read the rows: **both purchases are director Brian Healy's, 2026-05-15 and 2026-07-07, and both are flagged `is_10b5_1`.** MWA's true discretionary file over 180 days is **zero buyers and $75,000 of discretionary selling** (VP/CAO Feyerherm, 2026-08-26), alongside $2.72M of 10b5-1 and auto-plan selling including the CEO's $1.06M on 2026-08-10.

**What it did and did not change.** The gate outcome is unchanged: $75,000 of net discretionary selling is still below the $250k floor, so MWA is still not vetoed and the WATCHLIST verdict stands on its own two legs. **What changed is the character of the file**, from thin-but-two-sided to no discretionary buying at all, which is materially worse and which the morning brief reported the friendlier way. That is the same failure the 2026-08-27 writing rule addresses from the other direction: describe the file, and make sure the file being described is the real one.

**Fourth member of the same defect class**, and the class is now the most productive finding in this file: the 8/18 unstated veto window, the 8/26 unreported empty set, the 8/28 share-title string match, and now an unstated definition of "buying" on the same gate. **Three of the four are gate 7.** The general lesson has not changed and is now overdetermined: **every gate must name its measurement window, its universe, and its definitions, and must report what it measured.**

**Proposed, NOT adopted:** align the gate 7 buy leg with the INS path by excluding `is_10b5_1` as well as `is_auto_plan`, and report discretionary buyers and 10b5-1 buyers as separate counts so a zero-discretionary file is never again invisible. Queued for the monthly review with the widened share-title match and the gate 8 pairing fix. No verdict is re-cut on this today.
