...
updated: 2026-07-29
supersedes: Mobile Brief Prompt 2026-07-27.md
status: SOLE daily brief. The desktop Cowork daily-portfolio-check is retired.
---
...
Revision written 2026-07-29. Change from the 2026-07-27 revision: Kevin retired
the desktop Cowork brief, so this routine is now the only pre-market brief he
gets. Three consequences are baked in below.
...
- **An unconfirmed entry date makes the stop uncomputable.** Any entry date marked
  "(assumed)" means the peak-since-entry is unreliable and the stop is an artifact.
  Do not emit a stop signal. The valuation-target leg does NOT depend on entry
  date, so it still applies to these names. XRAY is the live case. List the name
