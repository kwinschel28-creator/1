...
| ISRG | Intuitive Surgical | ~1.58 | ~378.81 | 378.81 | 600 | 2.75% | A (dislocated quality) | Robotic surgery; hospital capital equipment + recurring instruments | In (medtech) |
...
**Cash:** ~$3,414 (15.6%), after the ADT exit, the ORCL theme-cap trim and the ISRG starter. First real dry powder since 7/02. No cash ceiling is set; see [[Portfolio Strategy]]. **Total book:** ~$21,820.
...
**Conviction provenance:** ISRG (2026-08-10) came from the agent's results-up/price-down screen, so it is BORROWED, and the three-question check was a partial pass only. GOOG is my own research. ORCL (Twitter), CRS (Perplexity screen), QXO (friend) are borrowed. ZTS, GEHC, KMX came from the 7/2 agent screens (borrowed until the operating work is done). PATK, NCLH came from the 7/16 agent insider-cluster screen; ADT, RYAN from the 7/23 agent insider-cluster screen (all four borrowed). NBIS, VSAT, VEEV, VRTX, XRAY, OKLO, IREN: unrecorded, FILL IN.
...
- ISRG (DRAFT 2026-08-10, agent-written, REPLACE THIS): US da Vinci procedure growth breaks below ~10% y/y, or hospital capital budgets force sustained discounting on system placements. **Kevin's own draft ("no innovation outside stomach surgeries") was rejected as unmonitorable: it rests on a wrong procedure mix, the volume base being urology, gynecology and general surgery, not stomach. Rewrite this in his own terms before it counts as pre-registered.**
