...
Paste the block below into the mobile Claude Code scheduled task, replacing the current prompt in full.

Revision written 2026-07-27 after auditing the 7/27 mobile output. Fixes, in order of severity: percent moves sourced from headlines instead of the price pull, month-old evidence presented as a fresh trigger fire, a missed catalyst calendar sweep, a trim signal on a name the vault said not to monitor, and unattributed ATR inputs feeding a mechanical rule.

## Repo prerequisites

Before this prompt works as written, run on the desktop:

```
cd "/Users/kevinwinschel/Documents/Claude/Projects/Stocks/Investment Process"
git add "Sell Discipline.md" "Checklist.md" "Philosophy.md" "Agent Context.md"
git commit -m "Sync process rules and agent context for mobile brief"
git push mobile-brief main
```

The repo must contain six files at root: `Holdings.md`, `Portfolio Strategy.md`, `Sell Discipline.md`, `Checklist.md`, `Philosophy.md`, `Agent Context.md`. Re-push after any vault edit that touches holdings, triggers, trim lines, caps, or the standing context.
...
The repo kwinschel28-creator/1 is checked out in your working directory. First read `Agent Context.md` from the repo root: it is your standing memory (Kevin's investing style, caps, brief and screen preferences, sourcing loop, known failure modes) and it governs how you write everything below. Then read `Holdings.md`, `Portfolio Strategy.md`, `Sell Discipline.md`, `Checklist.md`, and `Philosophy.md` from the repo root (use Glob/Read if the exact path is not obvious). If `Holdings.md` or `Portfolio Strategy.md` is missing, say so plainly at the top and stop rather than guessing at Kevin's positions. If `Agent Context.md` or any of the other three is missing, note it in one line under Data and continue.
...
- **Read the Notes column in the Gainer trim lines table and obey it.** A name marked immaterial, monitor-only-if-resized, or similar is excluded from the monitor entirely. It does not appear in the alert block, ever. QXO is the live case: it is 0.2% of book and marked "monitor only if resized." Do not compute its stop.
- **An unconfirmed entry date makes the stop uncomputable.** Any entry date marked "(assumed)" means the peak-since-entry is unreliable and the stop is an artifact. Do not emit a signal. List the name once in a Data-gaps footer line instead.
- Lane A names (GOOG, ORCL) are exempt. They trim only at the 20% ceiling or on a thesis break.
- Lane-unassigned names (marked "? (fill in)") are not on the monitor. Assign a lane first.
